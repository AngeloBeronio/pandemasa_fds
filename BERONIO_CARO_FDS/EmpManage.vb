...
Public Class EmpManage
    ...
    Private Sub LoadEmployees(Optional filterId As String = "", Optional filterRoleId As Integer = -1)
        Try
            ...
            Dim query As String = "
                SELECT e.employee_id, e.username, e.last_name, e.first_name, e.middle_initial,
                       r.role_name, e.salary, e.role_id
                FROM employees e
            ...
            Using reader As MySqlDataReader = cmd.ExecuteReader()
                While reader.Read()
                    ...
                    row.Cells("colUsername").Value = reader("username").ToString()
                    ...
                    row.Cells("colSalary").Value = $"₱{Convert.ToDecimal(reader("salary")):N2}"
    ...
    Private Sub btn_AddEmployee_Click(sender As Object, e As EventArgs) Handles btn_AddEmployee.Click
        ...
        If username = "" OrElse passcode = "" OrElse lastName = "" OrElse firstName = "" Then
            MsgWarn("Please fill out Username, Passcode, Last Name, and First Name.") : Return
        End If

        Try
            OpenConnection()
            Dim cmd As New MySqlCommand("
                INSERT INTO employees (username, passcode, last_name, first_name, middle_initial, role_id, salary)
                VALUES (@user, @pass, @last, @first, @mi, NULL, 0)", conn)
            ...
            LoadEmployees()
        Catch ex As MySqlException When ex.Number = 1062
            MsgWarn("That username already exists.")
        Catch ex As Exception
            MessageBox.Show($"Error: {ex.Message}")
        Finally
            CloseConnection()
    ...
    Private Sub btn_Set_Click(sender As Object, e As EventArgs) Handles btn_Set.Click
        ...
        Try
            OpenConnection()
            Dim cmd As New MySqlCommand("UPDATE employees SET role_id = @role, salary = @sal WHERE employee_id = @id", conn)
    ...
    Private Sub ClearAddForm()
        txt_Username.Clear() : txt_Passcode.Clear()
