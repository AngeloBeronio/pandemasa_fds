...
Public Class Admin_ManageProducts
    ...
    Private Sub LoadInventory()
        Try
            ...
            Dim query As String =
                ...
                "p.is_baked, " &
                "CASE WHEN p.is_baked = 1 THEN IFNULL(rc.recipe_cost, 0) " &
                "ELSE p.estimated_cost_price END AS cost_per_piece, " &
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If ofd.ShowDialog() = DialogResult.OK Then
            Try
                ...
                MessageBox.Show("Image attached: " & destFileName, "Ready",
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        If productName = "" Or ComboBox1.SelectedValue Is Nothing Then
            MessageBox.Show("Please fill out Name and Category options.",
        ...
        Try
            ...
            Dim cmd As New MySqlCommand(
                "INSERT INTO products (category_id, product_name, image_path, selling_price, estimated_cost_price, is_baked) " &
                "VALUES (@cat, @name, @img, 0.00, 0.00, @baked)", conn)
            ...
            cmd.ExecuteNonQuery()
            MessageBox.Show("Product added! Set prices and quantities next.",
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        Dim result = MessageBox.Show("Remove this item permanently?", "Confirm",
        ...
        Catch ex As MySqlException
            If ex.Number = 1451 Then
                MessageBox.Show("Cannot remove product with associated transactional sales history.",
    ...
    Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
        ...
        Try
            ...
            If selectedIsBaked Then
                Dim recipe As List(Of Tuple(Of Integer, String, Decimal)) =
                    GetProductIngredients(selectedProductId, conn, trans)

                If recipe.Count = 0 Then
                    trans.Rollback()
                    MessageBox.Show("This product is marked as Baked but has no recipe set up yet. " &
                                    "Please add ingredients first.",
                                    "Missing Recipe", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    Exit Sub
                End If
                ...
                For Each ing In recipe
                    ...
                    If currentStock < neededGrams Then
                        shortages.Add(ing.Item2 & " (need " & neededGrams.ToString("N1") &
                                      "g, have " & currentStock.ToString("N1") & "g)")
                    End If
                Next

                If shortages.Count > 0 Then
                    trans.Rollback()
                    MessageBox.Show("Cannot add stock — not enough ingredient stock:" &
                                    Environment.NewLine & String.Join(Environment.NewLine, shortages),
                                    "Insufficient Ingredients", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    Exit Sub
                End If
                ...
                For Each ing In recipe
                    ...
                    Dim deductCmd As New MySqlCommand(
                        "UPDATE ingredients SET stock_grams = stock_grams - @amt WHERE ingredient_id = @iid",
                        conn, trans)
            ...
            Dim upStock As New MySqlCommand(
            ...
            Dim upStatus As New MySqlCommand(
                ...
                "ELSE 'Available' END WHERE product_id = @id", conn, trans)
            upStatus.Parameters.AddWithValue("@id", selectedProductId)
            upStatus.ExecuteNonQuery()

            Dim logCmd As New MySqlCommand(
                "INSERT INTO productinv_logs (product_id, user_id, adjustment_quantity, adjustment_unit) " &
                "VALUES (@pid, @uid, @qty, @unit)", conn, trans)
            ...
            trans.Commit()
            MessageBox.Show("Stock adjusted! Added " & finalQtyToAdd & " individual units.",
        ...
        Try
            ...
            If isBakedValue Then
                cmd = New MySqlCommand(
                    "UPDATE products SET selling_price = @sp, is_baked = @baked WHERE product_id = @id", conn)
            Else
                cmd = New MySqlCommand(
                    "UPDATE products SET selling_price = @sp, estimated_cost_price = @cp, is_baked = @baked WHERE product_id = @id", conn)
            ...
            ApplyBakedFieldState()
            MessageBox.Show("Price adjustments recorded.", "Success",
                            MessageBoxButtons.OK, MessageBoxIcon.Information)
            LoadInventory()
        Catch ex As Exception
    ...
    Private Function GetProductIngredients(productId As Integer, conn As MySqlConnection,
                                           trans As MySqlTransaction) As List(Of Tuple(Of Integer, String, Decimal))
        Dim result As New List(Of Tuple(Of Integer, String, Decimal))
        Dim cmd As New MySqlCommand(
            "SELECT pi.ingredient_id, i.ingredient_name, pi.qty_gram_per_piece " &
            "FROM product_ingredients pi " &
            "JOIN ingredients i ON pi.ingredient_id = i.ingredient_id " &
    ...
    Private Function GetIngredientStock(ingredientId As Integer, conn As MySqlConnection,
                                        trans As MySqlTransaction) As Decimal
        Dim cmd As New MySqlCommand(
            "SELECT stock_grams FROM ingredients WHERE ingredient_id = @iid", conn, trans)
    ...
    Private Function GetIngredientIdByName(ingredientName As String, conn As MySqlConnection) As Object
        Dim cmd As New MySqlCommand(
            "SELECT ingredient_id FROM ingredients WHERE ingredient_name = @name LIMIT 1", conn)
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        If selectedProductId = -1 Then
            MessageBox.Show("Please select a product first.", "Missing Info",
        ...
        If ingName = "" Then
            MessageBox.Show("Please enter an ingredient name.", "Missing Info",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If

        If Not Decimal.TryParse(TextBox4.Text, gramsPerPiece) Then
            MessageBox.Show("Please enter a valid amount in grams.", "Missing Info",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If
        ...
        Try
            ...
            If ingredientId Is Nothing Then
                MessageBox.Show("Ingredient '" & ingName & "' was not found in the ingredients list.",
                                "Not Found", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If

            Dim existsCmd As New MySqlCommand(
                "SELECT COUNT(*) FROM product_ingredients WHERE product_id = @pid AND ingredient_id = @iid", conn)
            ...
            If alreadyExists > 0 Then
                Dim updateCmd As New MySqlCommand(
                    "UPDATE product_ingredients SET qty_gram_per_piece = @qty " &
                    "WHERE product_id = @pid AND ingredient_id = @iid", conn)
            ...
            Else
                Dim insertCmd As New MySqlCommand(
                    "INSERT INTO product_ingredients (product_id, ingredient_id, qty_gram_per_piece) " &
                    "VALUES (@pid, @iid, @qty)", conn)
            ...
            MessageBox.Show("Recipe ingredient saved.", "Success",
                            MessageBoxButtons.OK, MessageBoxIcon.Information)
            TextBox2.Clear()
            TextBox4.Clear()
            PopulateProductIngredientComboBox(selectedProductId)
    ...
    Private Sub Button10_Click(sender As Object, e As EventArgs) Handles Button10.Click
        ...
        Try
            OpenConnection()
            Dim cmd As New MySqlCommand(
                "DELETE FROM product_ingredients WHERE product_id = @pid AND ingredient_id = @iid", conn)
            cmd.Parameters.AddWithValue("@pid", selectedProductId)
            cmd.Parameters.AddWithValue("@iid", ComboBox2.SelectedValue)
            cmd.ExecuteNonQuery()
            MessageBox.Show("Ingredient removed from recipe.", "Success",
                            MessageBoxButtons.OK, MessageBoxIcon.Information)
            PopulateProductIngredientComboBox(selectedProductId)
        Catch ex As Exception
